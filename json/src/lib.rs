...
pub struct GetBlockTemplateOptions {
    pub mode: GetBlockTemplateOptionsModes,
    //// List of client side supported softfork deployment
    pub rules: Vec<GetBlockTemplateOptionsRules>,
    /// List of client side supported features
    pub capabilities: Vec<GetBlockTemplateOptionsCapabilities>,
}

/// Enum to represent client-side supported features
#[derive(Copy, Clone, PartialEq, Eq, Debug, Deserialize, Serialize)]
#[serde(rename_all = "lowercase")]
pub enum GetBlockTemplateOptionsCapabilities {
...
#[derive(Copy, Clone, PartialEq, Eq, Debug, Deserialize, Serialize)]
#[serde(rename_all = "lowercase")]
pub enum GetBlockTemplateOptionsRules {
    /// Inidcates that the requested template should support segwit blocks
    SegWit,
    /// Indicates that the requested template should support signet blocks
    Signet,
    /// Indicates that the reuested template should support CSV
    Csv,
    /// Indicates that the reuested template should support Taproot
    Taproot,
}
...
#[derive(Copy, Clone, PartialEq, Eq, Debug, Deserialize, Serialize)]
#[serde(rename_all = "lowercase")]
pub enum GetBlockTemplateOptionsModes {
